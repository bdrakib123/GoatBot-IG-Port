...
export default class DirectMessageV2 {
  ...
  async getFullInbox(maxPages = 5) {
    ...
    do {
      ...
      page++;
      if (inboxFeed.isMoreAvailable() && page < maxPages) await sleep(600);
  ...
  async getThreadIdByUsername(username) {
    ...
    const thread = inbox.threads.find(t =>
      t.users?.some(u => u.pk.toString() === userId.toString())
    );
  ...
  async sendMessage(threadId, text, options = {}) {
    return withRetry(async () => {
      ...
      if (itemId) this._trackSeen(itemId);
      ...
      onRetry: ({ attempt, delay, kind }) =>
        logger.warn(`sendMessage retry ${attempt} in ${(delay / 1000).toFixed(1)}s (${kind})`),
    });
  }
  ...
  async sendMessageBulk(threadIds, text, delayBetween = 1500) {
    ...
    for (let i = 0; i < threadIds.length; i++) {
      ...
      if (i < threadIds.length - 1 && delayBetween > 0) await sleep(delayBetween);
    }
    return results;
  }
  ...
  async getPendingInbox() {
    try {
      ...
      if (classifyError(error) === 'auth') throw error;
  ...
  async sendPhotoWithCaption(threadId, photoPath, caption = '') {
    ...
    if (caption) { await sleep(400); await this.sendMessage(threadId, caption); }
    return result;
  }
  ...
  async _downloadToTemp(url, ext, timeoutMs = 45000) {
    const response = await withTimeout(
      axios.get(url, { responseType: 'arraybuffer', timeout: timeoutMs, headers: { 'User-Agent': 'Mozilla/5.0' } }),
      timeoutMs + 5000, 'download'
    );
    ...
    if (message.media) {
      ...
      if (message.media.carousel_media) {
        out.media.carousel = message.media.carousel_media.map(i => {
          const c = { id: i.id };
          if (i.image_versions2) c.images = i.image_versions2.candidates;
          if (i.video_versions)  c.videos = i.video_versions;
          return c;
        });
      }
    }
  ...
  async forwardMessage(fromThreadId, toThreadId, itemId) {
    ...
    if (info.media?.videos)  return this.sendVideoFromUrl(toThreadId, info.media.videos[0].url);
    if (info.media?.images)  return this.sendPhotoFromUrl(toThreadId, info.media.images[0].url);
  ...
  async indicateTyping(threadId, isTyping = true) {
    try {
      if (isTyping) await this.ig.entity.directThread(threadId).broadcastTypingIndicator();
    } catch (_) { }
  }
    ...
    try {
      ...
      for (const thread of inbox.threads) {
        ...
        for (const item of (thread.items || [])) {
          if (item?.item_id) this._trackSeen(item.item_id);
        }
      }
      this.isSeeded = true;
  ...
  async startPolling(intervalOrOptions = 5000) {
    ...
    while (this.isPolling) {
      ...
      try {
        ...
        sweepCounter++;
        if (sweepCounter % 20 === 0) this._sweepExpiredReplyHandlers();
  ...
  async _pollCycle() {
    const inbox = await this.getInbox();
    if (!inbox.threads || inbox.threads.length === 0) return false;
    ...
    for (const thread of inbox.threads) {
      ...
      if (prevLastId === lastPerm.item_id) continue;
  ...
  _extractAttachments(item) {
      ...
      if (item.media) {
          ...
          if (url) attachments.push({ type, url });
      }
