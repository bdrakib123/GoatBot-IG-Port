  ...
  async voice(threadID, audioPath, options = {}, callback) {
    ...
    try {
      ...
      if (!response || response.status !== 'ok') {
        const fallbackUrls = [
          'https://i.instagram.com/api/v1/direct_v2/threads/broadcast/audio/',
          'https://www.instagram.com/api/v1/direct_v2/threads/broadcast/audio/'
        ];
        for (const fbUrl of fallbackUrls) {
          try {
            response = await this.http.postForm(fbUrl, messageData);
